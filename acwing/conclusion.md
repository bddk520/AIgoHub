...
* 一定要注意数组的下标是不是从0开始的，例如题目[529.宝藏](算法提高课\第一章动态规划\529.宝藏\529.宝藏.cpp)
...
* 注意输入和数据类型的对应，例如[524.愤怒的小鸟.cpp](算法提高课\第一章动态规划\524.愤怒的小鸟\524.愤怒的小鸟.cpp)定义成double型，scanf也要用%lf
* 注意double，int类型变量，注意精度问题
* 注意c++中尽量用unordered_map而不是map,unordered_map插入、删除和查找的时间复杂度为0（1），map是O（n)
* 注意区间dp要先遍历长度，再选择区间首部。如果遍历左右短点，或导致右侧区间会计算完，即子问题未完成就进行状态转移，这样计算错误，例如[479.加分二叉树](算法提高课\第一章动态规划\479.加分二叉树\479.加分二叉树.cpp)
